...
namespace VLB
{
    ...
    public partial class VolumetricLightBeam : MonoBehaviour
    {
        ...
        /// </summary>
        [FormerlySerializedAs("alphaInside")] public float intensityInside = Consts.Beam.IntensityDefault;

        [System.Obsolete("Use 'intensityGlobal' or 'intensityInside' instead")]
        public float alphaInside
        {
            get => intensityInside;
            set => intensityInside = value;
        }
        ...
        {
            get => intensityOutside;
            set => intensityOutside = value;
        }
...
#endif
                Debug.Assert(Utils.Approximately(intensityInside, intensityOutside),
                    "The beam is not using advanced intensity mode, but its inside and outside have not the same value.",
                    gameObject);
        ...
        public float coneAngle =>
            Mathf.Atan2(coneRadiusEnd - coneRadiusStart, maxGeometryDistance) * Mathf.Rad2Deg * 2f;

        /// <summary>
        /// Start radius of the cone geometry.
        /// 0 will generate a perfect cone geometry. Higher values will generate truncated cones.
        /// </summary>
        [FormerlySerializedAs("radiusStart")] public float coneRadiusStart = Consts.Beam.ConeRadiusStart;
        ...
        {
            get => Utils.ComputeConeRadiusEnd(maxGeometryDistance, spotAngle);
        ...
        public float coneVolume
        {
            get
            {
                float r1 = coneRadiusStart, r2 = coneRadiusEnd;
                return Mathf.PI / 3 * (r1 * r1 + r1 * r2 + r2 * r2) * fallOffEnd;
            }
        }
        ...
        public float coneApexOffsetZ
        {
            ...
            {
                // simple intercept
                var ratioRadius = coneRadiusStart / coneRadiusEnd;
                return ratioRadius == 1f ? float.MaxValue : maxGeometryDistance * ratioRadius / (1 - ratioRadius);
            }
        }

        public Vector3 coneApexPositionLocal => new Vector3(0, 0, -coneApexOffsetZ);
        public Vector3 coneApexPositionGlobal => transform.localToWorldMatrix.MultiplyPoint(coneApexPositionLocal);
        ...
        /// </summary>
        [FormerlySerializedAs("geomSides")] public int geomCustomSides = Consts.Beam.GeomSidesDefault;
        ...
        {
            get => geomMeshType == MeshType.Custom ? geomCustomSides : Config.Instance.sharedMeshSides;
            set
            {
                geomCustomSides = value;
                Debug.LogWarning(
                    "The setter VLB.VolumetricLightBeam.geomSides is OBSOLETE and has been renamed to geomCustomSides.");
            }
        }
        ...
        {
            get => geomMeshType == MeshType.Custom ? geomCustomSegments : Config.Instance.sharedMeshSegments;
            set
            {
                geomCustomSegments = value;
                Debug.LogWarning(
                    "The setter VLB.VolumetricLightBeam.geomSegments is OBSOLETE and has been renamed to geomCustomSegments.");
            }
        }
            ...
            get
            {
                if (Mathf.Approximately(skewingLocalForwardDirection.z, 0.0f))
                {
                    Debug.LogErrorFormat(
                        "Beam {0} has a skewingLocalForwardDirection with a null Z, which is forbidden", name);
        ...
        public Vector4 additionalClippingPlane => clippingPlaneTransform == null
            ? Vector4.zero
            : Utils.PlaneEquation(clippingPlaneTransform.forward, clippingPlaneTransform.position);
        ...
        {
            get => fallOffEndFromLight;
            set => fallOffEndFromLight = value;
        }
        ...
        /// </summary>
        /// 
        [FormerlySerializedAs("fadeStart")] public float fallOffStart = Consts.Beam.FallOffStart;

        [System.Obsolete("Use 'fallOffStart' instead")]
        public float fadeStart
        {
            get => fallOffStart;
            set => fallOffStart = value;
        }

        /// <summary>
        /// Distance from the light source (in units) the beam is entirely faded out.
        /// </summary>
        [FormerlySerializedAs("fadeEnd")] public float fallOffEnd = Consts.Beam.FallOffEnd;

        [System.Obsolete("Use 'fallOffEnd' instead")]
        public float fadeEnd
        {
            get => fallOffEnd;
            set => fallOffEnd = value;
        }
        ...
        /// </summary>
        [Range(0f, 1f)] public float glareFrontal = Consts.Beam.GlareFrontal;

        /// <summary>
        /// Boost intensity factor when looking at the beam from behind.
        /// </summary>
        [Range(0f, 1f)] public float glareBehind = Consts.Beam.GlareBehind;
        ...
        {
            get => isNoiseEnabled;
            set => noiseMode = value ? NoiseMode.WorldSpace : NoiseMode.Disabled;
        }
        ...
        /// </summary>
        public bool isFadeOutEnabled => _FadeOutBegin >= 0 && _FadeOutEnd >= 0;
        ...
        {
            get => SortingLayer.IDToName(sortingLayerID);
        ...
        {
            get => _TrackChangesDuringPlaytime;
            set
            {
        ...
        public Bounds bounds =>
            m_BeamGeom != null ? m_BeamGeom.meshRenderer.bounds : new Bounds(Vector3.zero, Vector3.zero);

        public int blendingModeAsInt =>
            Mathf.Clamp((int) blendingMode, 0, System.Enum.GetValues(typeof(BlendingMode)).Length);

        public Quaternion beamInternalLocalRotation => dimensions == Dimensions.Dim3D
            ? Quaternion.identity
            : Quaternion.LookRotation(Vector3.right, Vector3.up);

        public Vector3 beamLocalForward => dimensions == Dimensions.Dim3D ? Vector3.forward : Vector3.right;
        public Vector3 beamGlobalForward => transform.localToWorldMatrix.MultiplyVector(beamLocalForward);

        public Vector3 lossyScale => dimensions == Dimensions.Dim3D
            ? transform.lossyScale
            : new Vector3(transform.lossyScale.z, transform.lossyScale.y, transform.lossyScale.x);
        ...
        public Vector3 raycastGlobalUp => beamInternalLocalRotation * transform.up;
        public Vector3 raycastGlobalRight => beamInternalLocalRotation * transform.right;

        // INTERNAL
        public MaterialManager.DynamicOcclusion _INTERNAL_DynamicOcclusionMode
        {
            get => Config.Instance.featureEnabledDynamicOcclusion
                ? m_INTERNAL_DynamicOcclusionMode
                : MaterialManager.DynamicOcclusion.Off;
            set => m_INTERNAL_DynamicOcclusionMode = value;
        }

        public MaterialManager.DynamicOcclusion _INTERNAL_DynamicOcclusionMode_Runtime =>
            m_INTERNAL_DynamicOcclusionMode_Runtime
                ? _INTERNAL_DynamicOcclusionMode
                : MaterialManager.DynamicOcclusion.Off;
        ...
        public void RegisterOnBeamGeometryInitializedCallback(OnBeamGeometryInitialized cb)
        {
            ...
            if (m_BeamGeom) CallOnBeamGeometryInitializedCallback();
        }
...
#pragma warning restore 0414
        ...
        [FormerlySerializedAs("trackChangesDuringPlaytime")] [SerializeField]
        private bool _TrackChangesDuringPlaytime = false;

        [SerializeField] private int _SortingLayerID = 0;
        [SerializeField] private int _SortingOrder = 0;

        [FormerlySerializedAs("fadeOutBegin")] [SerializeField]
        private float _FadeOutBegin = Consts.Beam.FadeOutBeginDefault;

        [FormerlySerializedAs("fadeOutEnd")] [SerializeField]
        private float _FadeOutEnd = Consts.Beam.FadeOutEndDefault;

        private void SetFadeOutValue(ref float propToChange, float value)
        {
            var wasEnabled = isFadeOutEnabled;
            propToChange = value;
        ...
        private void OnFadeOutStateChanged()
        {
            ...
            if (isFadeOutEnabled && m_BeamGeom)
        ...
        public string meshStats
        {
            get
            {
                var mesh = m_BeamGeom ? m_BeamGeom.coneMesh : null;
                if (mesh)
                    return string.Format("Cone angle: {0:0.0} degrees\nMesh: {1} vertices, {2} triangles", coneAngle,
                        mesh.vertexCount, mesh.triangles.Length / 3);
                else return "no mesh available";
            }
        }

        public int meshVerticesCount => m_BeamGeom && m_BeamGeom.coneMesh ? m_BeamGeom.coneMesh.vertexCount : 0;

        public int meshTrianglesCount =>
            m_BeamGeom && m_BeamGeom.coneMesh ? m_BeamGeom.coneMesh.triangles.Length / 3 : 0;
        ...
        private Light lightSpotAttached
        {
            get
            {
                ...
                {
                    AttachedLightType lightType;
                    return GetLightSpotAttachedSlow(out lightType);
                }
#endif
        ...
        {
            if (dimensions == Dimensions.Dim2D) posOS = new Vector3(posOS.z, posOS.y, posOS.x);

            if (posOS.z < 0f) return -1f;

            var posOSXY = posOS.xy();

            if (hasMeshSkewing)
            {
                var localForwardDirN = skewingLocalForwardDirectionNormalized;
            ...
            var slopeRad = coneAngle * Mathf.Deg2Rad / 2;
        ...
        private void ClampProperties()
        {
            ...
            geomCustomSegments =
                Mathf.Clamp(geomCustomSegments, Consts.Beam.GeomSegmentsMin, Consts.Beam.GeomSegmentsMax);
