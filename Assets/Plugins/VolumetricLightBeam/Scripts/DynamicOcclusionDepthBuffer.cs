...
namespace VLB
{
    ...
    public class DynamicOcclusionDepthBuffer : DynamicOcclusionAbstractBase
    {
        ...
        private bool m_NeedToUpdateOcclusionNextFrame = false;
        ...
        protected override bool OnProcessOcclusion(ProcessOcclusionSource source)
        {
            ...
            if (source == ProcessOcclusionSource.RenderLoop &&
                SRPHelper.IsUsingCustomRenderPipeline()) // Recursive rendering is not supported on SRP
        ...
        private void UpdateDepthCameraPropertiesAccordingToBeam()
        {
            ...
            var apexDist = m_Master.coneApexOffsetZ;
        ...
        public bool HasLayerMaskIssues()
        {
            ...
            {
                var layerBit = 1 << Config.Instance.geometryLayerID;
...
#if UNITY_EDITOR
                    UnityEditor.GameObjectUtility.SetStaticEditorFlags(m_DepthCamera.gameObject,
...
#if UNITY_EDITOR
        private bool m_NeedToReinstantiateDepthCamera = false;

        private void MarkMaterialAsDirty()
        {
