...
namespace VLB
{
    ...
    public abstract class DynamicOcclusionAbstractBase : MonoBehaviour
    {
        ...
        protected void ProcessOcclusion(ProcessOcclusionSource source)
        {
            ...
            Debug.Assert(!Application.isPlaying || m_LastFrameRendered != Time.frameCount,
                "ProcessOcclusion has been called twice on the same frame, which is forbidden");
            Debug.Assert(m_Master);

            var occlusionSuccess = OnProcessOcclusion(source);
            ...
            if (m_Master)
            {
                ...
                m_Master._INTERNAL_SetDynamicOcclusionCallback(GetShaderKeyword(),
                    occlusionSuccess ? m_MaterialModifierCallbackCached : (MaterialModifier.Callback) null);
            }

            if (updateRate.HasFlag(DynamicOcclusionUpdateRate.OnBeamMove))
                m_TransformPacked = transform.GetWorldPacked();

            var firstTime = m_LastFrameRendered < 0;
            m_LastFrameRendered = Time.frameCount;

            if (firstTime && _INTERNAL_ApplyRandomFrameOffset)
                m_LastFrameRendered +=
                    Random.Range(0,
                        waitXFrames); // add a random offset to prevent from updating texture for all beams having the same wait value
        }

        private TransformUtils.Packed m_TransformPacked;
        private int m_LastFrameRendered = int.MinValue;
        public int _INTERNAL_LastFrameRendered => m_LastFrameRendered; // for unit tests
        ...
        protected virtual void OnEnable()
        {
...
#endif
            {
                ...
                if (!updateRate.HasFlag(DynamicOcclusionUpdateRate.Never))
                    m_Master.RegisterOnBeamGeometryInitializedCallback(() =>
        ...
        private void OnWillCameraRender(Camera cam)
        {
            ...
            if (cam != null && cam.enabled
                            && Time.frameCount !=
                            m_LastFrameRendered) // prevent from updating multiple times if there are more than 1 camera
            {
