...
namespace VLB
{
    ...
    public class Config : ScriptableObject
    {
        ...
        [FormerlySerializedAs("renderPipeline")] [SerializeField]
        private RenderPipeline _RenderPipeline = Consts.Config.GeometryRenderPipelineDefault;
        ...
        [FormerlySerializedAs("renderingMode")] [SerializeField]
        private RenderingMode _RenderingMode = Consts.Config.GeometryRenderingModeDefault;
...
#pragma warning disable 0162 // warning CS0162: Unreachable code detected
                if (renderingMode == RenderingMode.GPUInstancing && !BatchingHelper.isGpuInstancingSupported)
...
#pragma warning restore 0162
                ...
                if (renderPipeline != RenderPipeline.BuiltIn)
                    ...
                    if (renderingMode == RenderingMode.MultiPass)
                        return RenderingMode.SinglePass;
        ...
        public Transform fadeOutCameraTransform
        {
            get
            {
                if (m_CachedFadeOutCamera == null) ForceUpdateFadeOutCamera();
        ...
        /// </summary>
        [HighlightNull] public Texture3D noiseTexture3D = null;

        /// <summary>
        /// ParticleSystem prefab instantiated for the Volumetric Dust Particles feature (Unity 5.5 or above)
        /// </summary>
        [HighlightNull] public ParticleSystem dustParticlesPrefab = null;
        ...
        public FeatureEnabledColorGradient featureEnabledColorGradient =
            Consts.Config.FeatureEnabledColorGradientDefault;
...
#pragma warning disable 0414
        ...
        [SerializeField] private Material _DummyMaterial = null;
#pragma warning restore 0414

        [SerializeField] private Shader _BeamShader = null;
        private Transform m_CachedFadeOutCamera = null;

        public bool hasRenderPipelineMismatch => SRPHelper.renderPipelineType == SRPHelper.RenderPipeline.BuiltIn !=
                                                 (_RenderPipeline == RenderPipeline.BuiltIn);
        ...
        private static void OnStartup()
        {
            ...
            if (Instance.hasRenderPipelineMismatch)
                Debug.LogError(
                    "It looks like the 'Render Pipeline' is not correctly set in the config. Please make sure to select the proper value depending on your pipeline in use.",
                    Instance);
        }
        ...
        {
            Shader.SetGlobalFloat(ShaderProperties.GlobalUsesReversedZBuffer,
        ...
        private void OnValidate()
        {
            ...
            sharedMeshSegments =
                Mathf.Clamp(sharedMeshSegments, Consts.Beam.GeomSegmentsMin, Consts.Beam.GeomSegmentsMax);

            ditheringFactor = Mathf.Clamp01(ditheringFactor);
        }

        private void AutoSelectRenderPipeline()
        {
        ...
        public void RefreshShader(RefreshShaderFlags flags)
        {
            if (flags.HasFlag(RefreshShaderFlags.Reference))
            {
                ...
                if (_BeamShader != prevShader) EditorUtility.SetDirty(this);
            }

            if (flags.HasFlag(RefreshShaderFlags.Dummy) && _BeamShader != null)
            {
                var gpuInstanced = actualRenderingMode == RenderingMode.GPUInstancing;
        ...
        public ParticleSystem NewVolumetricDustParticles()
        {
            ...
            {
                if (Application.isPlaying) Debug.LogError("Failed to instantiate VolumetricDustParticles prefab.");
                return null;
            }
        ...
        public static Config Instance => GetInstance(true);

#if UNITY_EDITOR
        private static bool ms_IsCreatingInstance = false;

        public bool IsCurrentlyUsedInstance()
        {
...
#endif
            {
                ...
                if (ms_IsCreatingInstance)
                {
                    Debug.LogError(string.Format(
                        "Trying to access Config.Instance while creating it. Breaking before infinite loop."));
                    return null;
                }
#endif
                {
                    var newInstance =
                        Resources.Load<ConfigOverride>(ConfigOverride.kAssetName +
                    ...
                    if (newInstance && newInstance != ms_Instance)
                    {
                        ...
                        newInstance
                            .RefreshGlobalShaderProperties(); // make sure noise textures are properly loaded as soon as the editor is started
                    }
#endif
                ...
                if (ms_Instance == null)
                {
                    ...
                    if (Application.isPlaying)
                        ms_Instance
                            .Reset(); // Reset is not automatically when instancing a ScriptableObject when in playmode
#endif
                    Debug.Assert(!(assertIfNotFound && ms_Instance == null),
                        string.Format(
                            "Can't find any resource of type '{0}'. Make sure you have a ScriptableObject of this type in a 'Resources' folder.",
                            typeof(Config)));
                }
            }
