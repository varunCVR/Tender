...
namespace VLB
{
    public static class MeshGenerator
    {
        ...
        public static Mesh GenerateConeZ_RadiusAndAngle(float lengthZ, float radiusStart, float coneAngle, int numSides,
            ...
            int numSegments, bool cap, bool doubleSided)
        {
            ...
            var vertCountSides = numSides * (numSegments + 2);
            var vertCountTotal = vertCountSides;
            ...
            // VERTICES
            {
                var angleOffset = GetAngleOffset(numSides);

                var vertices = new Vector3[vertCountTotal];

                for (var i = 0; i < numSides; i++)
                {
                    var angle = angleOffset + 2 * Mathf.PI * i / numSides;
                    var angleCos = Mathf.Cos(angle);
                    var angleSin = Mathf.Sin(angle);

                    for (var seg = 0; seg < numSegments + 2; seg++)
                    {
                        var tseg = (float) seg / (numSegments + 1);
                        Debug.Assert(tseg >= 0f && tseg <= 1f);
                        var radius = Mathf.Lerp(radiusStart, radiusEnd, tseg);
            ...
            // Y: 0 = front face ; 1 = back face (doubleSided only)
            {
                ...
                var ind = 0;
                for (var i = 0; i < vertCountSides; i++)
                    uv[ind++] = Vector2.zero;
            ...
            // INDICES
            {
                var triCountSides = numSides * 2 * Mathf.Max(numSegments + 1, 1);
                var indCountSides = triCountSides * 3;
                var indCountTotal = indCountSides;
        ...
        {
            var maxDiameter = Mathf.Max(radiusStart, radiusEnd) * 2;
            return new Bounds(
        ...
        public static int GetVertexCount(int numSides, int numSegments, bool geomCap, bool doubleSided)
        {
            ...
            var count = numSides * (numSegments + 2);
        ...
        public static int GetIndicesCount(int numSides, int numSegments, bool geomCap, bool doubleSided)
        {
            ...
            var count = numSides * (numSegments + 1) * 2 * 3;
        ...
        public static int GetSharedMeshVertexCount()
        {
            return GetVertexCount(Config.Instance.sharedMeshSides, Config.Instance.sharedMeshSegments, true,
                Config.Instance.requiresDoubleSidedMesh);
        }
