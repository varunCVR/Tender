...
namespace VLB
{
    public static class Utils
    {
        ...
        public static bool HasFlag(this Enum mask, Enum flags) // Same behavior than Enum.HasFlag is .NET 4
        {
            ...
            if (mask.GetType() != flags.GetType())
                throw new ArgumentException(string.Format(
                    "The argument type, '{0}', is not the same as the enum type '{1}'.", flags.GetType(),
                    mask.GetType()));
#endif
        ...
        public static bool Approximately(this float a, float b, float epsilon = kEpsilon)
        {
            return Mathf.Abs(a - b) < epsilon;
        }

        public static bool Approximately(this Vector2 a, Vector2 b, float epsilon = kEpsilon)
        {
            return Vector2.SqrMagnitude(a - b) < epsilon;
        }

        public static bool Approximately(this Vector3 a, Vector3 b, float epsilon = kEpsilon)
        {
            return Vector3.SqrMagnitude(a - b) < epsilon;
        }

        public static bool Approximately(this Vector4 a, Vector4 b, float epsilon = kEpsilon)
        {
        ...
        {
            return Mathf.Max(Mathf.Max(self.size.x * self.size.y, self.size.y * self.size.z),
...
#if UNITY_EDITOR
        public static void GizmosDrawPlane(Vector3 normal, Vector3 position, Color color, Matrix4x4 mat, float size =
            1f, float normalLength = 0.0f)
        {
            ...
            UnityEditor.Handles.RectangleHandleCap(0, position, Quaternion.LookRotation(normal), size,
                EventType.Repaint);

            if (normalLength > 0.0f)
            {
                UnityEditor.Handles.DrawLine(position, position + normal * normalLength);
                UnityEditor.Handles.ConeHandleCap(0, position + normal * normalLength, Quaternion.LookRotation(normal),
                    normalLength * 0.25f, EventType.Repaint);
            }
        ...
        {
            var iVal = Vector4_Floor(color * (floatPackingPrecision - 1));
        ...
        public static FloatPackingPrecision GetFloatPackingPrecision()
        {
            if (ms_FloatPackingPrecision == FloatPackingPrecision.Undef)
                ms_FloatPackingPrecision = SystemInfo.graphicsShaderLevel >= kFloatPackingHighMinShaderLevel
                    ? FloatPackingPrecision.High
                    : FloatPackingPrecision.Low;
        ...
        {
#if UNITY_EDITOR
            if (!Application.isPlaying) UnityEditor.EditorUtility.SetDirty(obj);
#endif
        }
        ...
        public static bool IsEditorCamera(Camera cam)
        {
            ...
            if (sceneView) return cam == sceneView.camera;
            return false;
        }
        ...
        public static void SetSameSceneVisibilityStatesThan(this GameObject self, GameObject model)
        {
            ...
            var pickingDisabled = UnityEditor.SceneVisibilityManager.instance.IsPickingDisabled(model);
            ...
            var hidden = UnityEditor.SceneVisibilityManager.instance.IsHidden(model);
