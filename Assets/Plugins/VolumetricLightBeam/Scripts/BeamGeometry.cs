...
namespace VLB
{
    ...
    public class BeamGeometry : MonoBehaviour, MaterialModifier.Interface
    {
        ...
        private MaterialModifier.Callback m_MaterialModifierCallback = null;
        private Coroutine m_CoFadeOut = null;
        ...
        {
            get => meshRenderer.sortingLayerID;
        ...
        private float ComputeFadeOutFactor(Transform camTransform)
        {
            ...
            {
                var distanceCamToBeam = Vector3.SqrMagnitude(meshRenderer.bounds.center - camTransform.position);
                return Mathf.InverseLerp(m_Master.fadeOutEnd * m_Master.fadeOutEnd,
                    m_Master.fadeOutBegin * m_Master.fadeOutBegin, distanceCamToBeam);
            }
            else
            {
                return 1.0f;
        ...
        private void ComputeFadeOutFactor()
        {
            ...
            if (camTransform)
            {
                var fadeOutFactor = ComputeFadeOutFactor(camTransform);
        ...
        public void RestartFadeOutCoroutine()
        {
...
#endif
            {
                ...
                if (m_Master && m_Master.isFadeOutEnabled) m_CoFadeOut = StartCoroutine(CoUpdateFadeOut());
            }
        }
...
#if VLB_SRP_SUPPORT
        private Camera m_CurrentCameraRenderingSRP = null;
        ...
        private bool shouldUseGPUInstancedMaterial =>
            m_Master._INTERNAL_DynamicOcclusionMode !=
            MaterialManager.DynamicOcclusion.DepthTexture // sampler cannot be passed to shader as instanced property
        ...
        public void Initialize(VolumetricLightBeam master)
        {
            ...
            meshRenderer.reflectionProbeUsage =
                UnityEngine.Rendering.ReflectionProbeUsage
                    .Off; // different reflection probes could break batching with GPU Instancing
            ...
            if (SortingLayer.IsValid(m_Master.sortingLayerID))
                ...
                Debug.LogError(string.Format(
                    "Beam '{0}' has an invalid sortingLayerID ({1}). Please fix it by setting a valid layer.",
                    Utils.GetPath(m_Master.transform), m_Master.sortingLayerID));
        ...
        public void RegenerateMesh()
        {
            ...
            if (coneMesh && m_CurrentMeshType == MeshType.Custom) DestroyImmediate(coneMesh);
            ...
            switch (m_Master.geomMeshType)
            {
                ...
                {
                    coneMesh = MeshGenerator.GenerateConeZ_Radius(1f, 1f, 1f, m_Master.geomCustomSides,
                        m_Master.geomCustomSegments, m_Master.geomCap, Config.Instance.requiresDoubleSidedMesh);
        ...
        private bool isNoiseEnabled =>
            m_Master.isNoiseEnabled && m_Master.noiseIntensity > 0f &&
            Noise3D.isSupported; // test Noise3D.isSupported the last

#pragma warning disable 0162
        private bool isDepthBlendEnabled => BatchingHelper.forceEnableDepthBlend || m_Master.depthBlendDistance > 0f;
#pragma warning restore 0162
        ...
        private bool ApplyMaterial()
        {
            ...
            if (m_Master.colorMode == ColorMode.Gradient)
            {
                ...
                colorGradient = precision == Utils.FloatPackingPrecision.High
                    ? MaterialManager.ColorGradient.MatrixHigh
                    : MaterialManager.ColorGradient.MatrixLow;
            }
            ...
            Debug.Assert((int) BlendingMode.TraditionalTransparency ==
                         (int) MaterialManager.BlendingMode.TraditionalTransparency);
            ...
            var staticProps = new MaterialManager.StaticProperties
            {
                ...
                meshSkewing =
                    m_Master.hasMeshSkewing ? MaterialManager.MeshSkewing.On : MaterialManager.MeshSkewing.Off,
                shaderAccuracy = m_Master.shaderAccuracy == ShaderAccuracy.Fast
                    ? MaterialManager.ShaderAccuracy.Fast
                    : MaterialManager.ShaderAccuracy.High
            };
            ...
            {
                mat = MaterialManager.GetInstancedMaterial(m_Master._INTERNAL_InstancedMaterialGroupID,
                    ref staticProps);
            }
        ...
        public void UpdateMaterialAndBounds()
        {
            ...
            MaterialChangeStart();
            {
                ...
                var slopeRad =
                    m_Master.coneAngle * Mathf.Deg2Rad /
                    2; // use coneAngle (instead of spotAngle) which is more correct with the geometry
                SetMaterialProp(ShaderProperties.ConeSlopeCosSin,
                    new Vector2(Mathf.Cos(slopeRad), Mathf.Sin(slopeRad)));
                ...
                var coneRadius = new Vector2(Mathf.Max(m_Master.coneRadiusStart, kMinRadius),
                    Mathf.Max(m_Master.coneRadiusEnd, kMinRadius));
                SetMaterialProp(ShaderProperties.ConeRadius, coneRadius);

                const float kMinApexOffset = 0.0001f;
                var nonNullApex = Mathf.Sign(m_Master.coneApexOffsetZ) *
                                  Mathf.Max(Mathf.Abs(m_Master.coneApexOffsetZ), kMinApexOffset);
                ...
                SetMaterialProp(ShaderProperties.DistanceFallOff,
                    new Vector3(m_Master.fallOffStart, m_Master.fallOffEnd, m_Master.maxGeometryDistance));
                ...
                SetMaterialProp(ShaderProperties.FresnelPow,
                    Mathf.Max(0.001f,
                        m_Master.fresnelPow)); // no pow 0, otherwise will generate inf fresnel and issues on iOS
                ...
                if (isNoiseEnabled)
                {
                    ...
                    var noiseVelocity = m_Master.noiseVelocityUseGlobal
                        ? Config.Instance.globalNoiseVelocity
                        : m_Master.noiseVelocityLocal;
                    var noiseScale = m_Master.noiseScaleUseGlobal
                        ? Config.Instance.globalNoiseScale
            ...
            for (int vertexInd = 0; vertexInd < coneMesh.vertexCount; vertexInd++)
            {
                ...
                var normal =
 new Vector3(cosSinFlat.x * Mathf.Cos(slopeRad), cosSinFlat.y * Mathf.Cos(slopeRad), -Mathf.Sin(slopeRad)).normalized;
        ...
        private void OnBeginCameraRenderingSRP(UnityEngine.Rendering.ScriptableRenderContext context, Camera cam)
#else
        ...
        private void OnWillCameraRenderThisBeam(Camera cam)
        {
            if (m_Master && cam)
                if (
#if UNITY_EDITOR
                    Utils.IsEditorCamera(
                        cam) || // make sure to call UpdateCameraRelatedProperties for editor scene camera 
#endif
        ...
        private void UpdateCameraRelatedProperties(Camera cam)
        {
            if (cam && m_Master)
            {
                MaterialChangeStart();
                {
                    ...
                    var camIsInsideBeamFactor =
                        cam.orthographic ? -1f : m_Master.GetInsideBeamFactorFromObjectSpacePos(camPosOS);
                    SetMaterialProp(ShaderProperties.CameraParams,
                        new Vector4(camForwardVectorOSN.x, camForwardVectorOSN.y, camForwardVectorOSN.z,
                            camIsInsideBeamFactor));
...
#if UNITY_EDITOR
        public bool _EDITOR_IsUsingCustomMaterial => m_CustomMaterial != null;
#endif
    }
}