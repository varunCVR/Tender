...
namespace VLB
{
    ...
    public class VolumetricDustParticles : MonoBehaviour
    {
        ...
        [SerializeField] private float m_AlphaAdditionalRuntime = 1.0f;

        public float alphaAdditionalRuntime
        {
            get => m_AlphaAdditionalRuntime;
            set
            {
        ...
        public int particlesCurrentCount => m_Particles ? m_Particles.particleCount : 0;
        public int particlesMaxCount => m_Particles ? m_Particles.main.maxParticles : 0;
                ...
                if (!ms_MainCamera)
                {
                    ...
                    if (!ms_MainCamera && !ms_NoMainCameraLogged)
                    {
                        Debug.LogErrorFormat(gameObject,
                            "In order to use '" + ClassName +
                            "' culling, you must have a MainCamera defined in your scene.");
        ...
        private void InstantiateParticleSystem()
        {
            ...
            if (m_Particles)
            {
                ...
                if (m_Master)
                {
                    UnityEditor.GameObjectUtility.SetStaticEditorFlags(m_Particles.gameObject,
                        m_Master.GetStaticEditorFlagsForSubObjects());
        ...
        private void Update()
        {
            ...
            {
                m_Material.SetColor(ShaderProperties.ParticlesTintColor,
                    new Color(1.0f, 1.0f, 1.0f, alphaAdditionalRuntime));
        ...
        private void SetParticleProperties()
        {
            if (m_Particles && m_Particles.gameObject.activeSelf)
            {
                ...
                var maxParticles = (int) (ratePerSec * 4);
...

                {
                    ...
                    velocityOverLifetime.space = direction == ParticlesDirection.LocalSpace
...

                {
                    ...
                    var coneAngle = m_Master.coneAngle * Mathf.Lerp(0.7f, 1f, thickness);
                    shape.angle = coneAngle * 0.5f;

                    var radiusStart = m_Master.coneRadiusStart * Mathf.Lerp(0.3f, 1.0f, thickness);
                    var radiusEnd = Utils.ComputeConeRadiusEnd(m_Master.fallOffEnd, coneAngle);
...
#pragma warning disable 0618
            ...
            if (serializedVersion < 1940) spawnDistanceRange = new MinMaxRangeFloat(spawnMinDistance, spawnMaxDistance);
