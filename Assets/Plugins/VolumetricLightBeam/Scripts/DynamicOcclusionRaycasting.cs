...
namespace VLB
{
    ...
    public class DynamicOcclusionRaycasting : DynamicOcclusionAbstractBase
    {
        ...
        {
            get => fadeDistanceToSurface;
            set => fadeDistanceToSurface = value;
        }
        ...
        public bool IsColliderHiddenByDynamicOccluder(Collider collider)
        {
            Debug.Assert(collider,
                "You should pass a valid Collider to VLB.DynamicOcclusion.IsColliderHiddenByDynamicOccluder");
        ...
        public struct HitResult
        {
            ...
            public bool hasCollider => collider2D || collider3D;
...
#if UNITY_EDITOR
        public HitResult editorCurrentHitResult => m_CurrentHit;
        ...
        public static void EditorLoadPrefs()
        {
            ...
            {
                editorShowDebugPlane =
                    UnityEditor.EditorPrefs.GetBool(EditorPrefsStrings.DynOcclusion.PrefShowDebugPlane, true);
                editorRaycastAtEachFrame =
                    UnityEditor.EditorPrefs.GetBool(EditorPrefsStrings.DynOcclusion.PrefRaycastingEditor, true);
        ...
        private void Start()
        {
            if (Application.isPlaying)
            {
                ...
                if (triggerZone) m_RangeMultiplier = Mathf.Max(1f, triggerZone.rangeMultiplier);
            }
        }
        ...
        private Vector3 GetRandomVectorAround(Vector3 direction, float angleDiff)
        {
            ...
            return Quaternion.Euler(Random.Range(-halfAngle, halfAngle), Random.Range(-halfAngle, halfAngle),
                Random.Range(-halfAngle, halfAngle)) * direction;
        }

        private QueryTriggerInteraction queryTriggerInteraction =>
            considerTriggers ? QueryTriggerInteraction.Collide : QueryTriggerInteraction.Ignore;
        ...
        {
            var hits = Physics2D.RaycastAll(new Vector2(rayPos.x, rayPos.y), new Vector2(rayDir.x, rayDir.y),
                raycastMaxDistance, layerMask.value);
            ...
            {
                if (!considerTriggers &&
                    hits[i].collider.isTrigger) // do not query triggers if considerTriggers is disabled
                    continue;
        ...
        private uint m_PrevNonSubHitDirectionId = 0;

        private uint GetDirectionCount()
        {
            return dimensions == Dimensions.Dim2D ? (uint) Direction.Max2D + 1 : (uint) Direction.Max3D + 1;
        }
        ...
        private bool IsHitValid(ref HitResult hit, Vector3 forwardVec)
        {
            if (hit.hasCollider)
            {
                var dot = Vector3.Dot(hit.normal, -forwardVec);
            ...
            if (IsHitValid(ref bestHit, raycastGlobalForward))
            {
                if (minSurfaceRatio > 0.5f)
                {
                    ...
                    for (uint i = 0; i < GetDirectionCount(); i++)
                    {
                        ...
                        var newPt = transform.position + dir3 * m_Master.coneRadiusEnd +
                                    raycastGlobalForward * raycastDistance;

                        var bestHitSub = GetBestHit(startPt, (newPt - startPt).normalized);
                        if (IsHitValid(ref bestHitSub, raycastGlobalForward))
                        {
                            if (bestHitSub.distance > bestHit.distance) bestHit = bestHitSub;
                        }
                        else
                        {
        ...
        protected override void OnModifyMaterialCallback(MaterialModifier.Interface owner)
        {
            ...
            owner.SetMaterialProp(ShaderProperties.DynamicOcclusionClippingPlaneWS,
                new Vector4(planeWS.normal.x, planeWS.normal.y, planeWS.normal.z, planeWS.distance));
        ...
        private Plane m_DebugPlaneLocal;
        ...
        private void OnDrawGizmos()
        {
            ...
            if (m_DebugPlaneLocal.IsValid())
            {
                ...
                var planeDistNormalized =
                    Mathf.Clamp01(Mathf.InverseLerp(0f, m_Master.raycastDistance, m_DebugPlaneLocal.distance));
                var planeSize = Mathf.Lerp(m_Master.coneRadiusStart, m_Master.coneRadiusEnd, planeDistNormalized);
