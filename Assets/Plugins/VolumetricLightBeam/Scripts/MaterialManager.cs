...
namespace VLB
{
    public static class MaterialManager
    {
        ...
        private static readonly UnityEngine.Rendering.BlendMode[] BlendingMode_SrcFactor =
            new UnityEngine.Rendering.BlendMode[(int) BlendingMode.Count]
            {
        ...
        private static readonly bool[] BlendingMode_AlphaAsBlack = new bool[(int) BlendingMode.Count]
        {
            true, // Additive
            true, // SoftAdditive
        ...
        private static int kStaticPropertiesCount = (int) BlendingMode.Count * (int) Noise3D.Count *
                                                    (int) DepthBlend.Count * (int) ColorGradient.Count *
                                                    (int) DynamicOcclusion.Count * (int) MeshSkewing.Count *
                                                    (int) ShaderAccuracy.Count;
        ...
        public struct StaticProperties
        {
            ...
            private int noise3DID => Config.Instance.featureEnabledNoise3D ? (int) noise3D : 0;
            private int depthBlendID => Config.Instance.featureEnabledDepthBlend ? (int) depthBlend : 0;

            private int colorGradientID =>
                Config.Instance.featureEnabledColorGradient != FeatureEnabledColorGradient.Off
                    ? (int) colorGradient
                    : 0;

            private int dynamicOcclusionID =>
                Config.Instance.featureEnabledDynamicOcclusion ? (int) dynamicOcclusion : 0;

            private int meshSkewingID => Config.Instance.featureEnabledMeshSkewing ? (int) meshSkewing : 0;
            private int shaderAccuracyID => Config.Instance.featureEnabledShaderAccuracyHigh ? (int) shaderAccuracy : 0;
            ...
            public void ApplyToMaterial(Material mat)
            {
                ...
                mat.SetKeywordEnabled(ShaderKeywords.ColorGradientMatrixHigh,
                    colorGradient == ColorGradient.MatrixHigh);
                ...
                mat.SetKeywordEnabled(ShaderKeywords.OcclusionClippingPlane,
                    dynamicOcclusion == DynamicOcclusion.ClippingPlane);
                mat.SetKeywordEnabled(ShaderKeywords.OcclusionDepthTexture,
                    dynamicOcclusion == DynamicOcclusion.DepthTexture);
        ...
        public static Material
            GetInstancedMaterial(uint groupID,
                ref StaticProperties staticProps) // pass StaticProperties by ref to avoid per value arg copy
        {
            var group = (MaterialsGroup) ms_MaterialsGroup[groupID];
            if (group == null)
            {
