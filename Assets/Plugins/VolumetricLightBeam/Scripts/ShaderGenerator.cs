...
namespace VLB
{
    ...
    {
        [SerializeField] private TextAsset m_Base = null;
        private TextAsset textAssetBase => m_Base;

        [SerializeField] private TextAsset m_Pass = null;
        private TextAsset textAssetPass => m_Pass;

        [SerializeField] private TextAsset m_IncludesBuiltin = null;
        private TextAsset textAssetIncludesBuiltin => m_IncludesBuiltin;

        [SerializeField] private TextAsset m_IncludesURP = null;
        private TextAsset textAssetIncludesURP => m_IncludesURP;

        [SerializeField] private TextAsset m_IncludesHDRP = null;
        private TextAsset textAssetIncludesHDRP => m_IncludesHDRP;

        private const string kShaderAssetName = "VLBGeneratedShader";
        ...
        private static string LoadText(TextAsset textAsset)
        {
            Debug.Assert(textAsset != null,
                "Fail to load a TextAsset, please try to reinstall the VolumetricLightBeam plugin");
            return textAsset.text;
        }

        private TextAsset GetTextAssetIncludes(SRPHelper.RenderPipeline rp)
        {
            switch (rp)
            {
        ...
        private static bool IsFogSupported(SRPHelper.RenderPipeline rp)
        {
            return rp != SRPHelper.RenderPipeline.HDRP;
        }

        private enum ShaderLangage
        {
            CG,
            HLSL
        }

        private static ShaderLangage GetShaderLangage(SRPHelper.RenderPipeline rp)
        {
            switch (rp)
            {
                case SRPHelper.RenderPipeline.BuiltIn:
        ...
        private static string GetShaderLangagePre(ShaderLangage lang)
        {
            return lang == ShaderLangage.CG ? "CGPROGRAM" : "HLSLPROGRAM";
        }

        private static string GetShaderLangagePost(ShaderLangage lang)
        {
            return lang == ShaderLangage.CG ? "ENDCG" : "ENDHLSL";
        }
        ...
        public class GenPass
        {
            ...
            private static void AppendMultiCompile(ref string str, bool genDefaultVariant, params string[] options)
            {
                ...
                foreach (var opt in options) str += " " + opt;
                str += System.Environment.NewLine;
            }

            public string Generate(SRPHelper.RenderPipeline rp, RenderingMode rm, EnabledFeatures enabledFeatures,
                int passID, int passCount)
            {
                var code = LoadText(Instance.textAssetPass);

                code = code.Replace("{VLB_GEN_CULLING}", m_CullMode.ToString());
                code = code.Replace("{VLB_GEN_PRAGMA_INSTANCING}",
                    rm == RenderingMode.GPUInstancing ? "#pragma multi_compile_instancing" : "");
                code = code.Replace("{VLB_GEN_PRAGMA_FOG}", IsFogSupported(rp) ? "#pragma multi_compile_fog" : "");

                var multiCompileVariants = "";
                ...
                {
                    case FeatureEnabledColorGradient.HighOnly:
                        AppendMultiCompile(ref multiCompileVariants, true, ShaderKeywords.ColorGradientMatrixHigh);
                        break;
                    case FeatureEnabledColorGradient.HighAndLow:
                        AppendMultiCompile(ref multiCompileVariants, true, ShaderKeywords.ColorGradientMatrixHigh,
                            ShaderKeywords.ColorGradientMatrixLow);
                        break;
                }

                if (enabledFeatures.dynamicOcclusion)
                    AppendMultiCompile(ref multiCompileVariants, true, ShaderKeywords.OcclusionClippingPlane,
                        ShaderKeywords.OcclusionDepthTexture);
                ...
                else
                {
                    ...
                    code = code.Replace("{VLB_GEN_INPUT_FS}",
                        string.Format("{0}", "i.cameraPosObjectSpace_outsideBeam.w"));
                    passPre += "                #define VLB_PASS_OUTSIDEBEAM_FROM_VS_TO_FS 1" +
        ...
        private class GenShader
        {
            private SRPHelper.RenderPipeline m_RenderPipeline;
            private RenderingMode m_RenderingMode;
            private EnabledFeatures m_EnabledFeatures;
            private List<GenPass> m_Passes = new List<GenPass>();
            ...
            private GenShader AddPass(CullMode cullMode)
            {
            ...
            public Shader Generate()
            {
                ...
                var passes = "";
                for (var i = 0; i < m_Passes.Count; ++i)
                    passes += m_Passes[i].Generate(m_RenderPipeline, m_RenderingMode, m_EnabledFeatures, i,
                        m_Passes.Count);
                ...
                var outputFolderPath = GetOutputPath();
                var outputFilePath = Path.Combine(outputFolderPath, kShaderAssetName);
                var outputFullPath = Path.Combine(Application.dataPath, outputFilePath + ".shader");
                File.WriteAllText(outputFullPath, code);
                AssetDatabase.Refresh();

                var shader = Shader.Find(shaderName);
                Debug.Assert(shader != null,
                    string.Format("Failed to generate shader '{0}' at '{1}'", shaderName, outputFullPath));
                return shader;
            }

            private string GetRenderPipelineInclude(SRPHelper.RenderPipeline rp)
            {
                switch (rp)
                {
        ...
        private static ShaderGenerator FindInstance()
        {
        ...
        private static GenerationParam ms_GenerationParamOnEnable = null;
        ...
        private void OnEnable()
        {
            if (ms_GenerationParamOnEnable != null && Instance != null)
            {
                Generate(ms_GenerationParamOnEnable.renderPipeline, ms_GenerationParamOnEnable.renderingMode,
                    ms_GenerationParamOnEnable.enabledFeatures);
                ms_GenerationParamOnEnable = null;
            }
        }

        // Singleton management
        private static ShaderGenerator m_Instance = null;
