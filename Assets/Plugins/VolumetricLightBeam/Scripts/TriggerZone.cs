    ...
    public class TriggerZone : MonoBehaviour
    {
        ...
        private TriggerZoneUpdateRate updateRate
        {
            get
            {
                ...
                if (m_Beam.dimensions == Dimensions.Dim3D)
                    return
                        TriggerZoneUpdateRate
                            .OnEnable; // for 3D meshes, do it only once because it's too performance heavy
                return m_DynamicOcclusionRaycasting != null
                    ? TriggerZoneUpdateRate.OnOcclusionChange
                    : TriggerZoneUpdateRate.OnEnable;
            }
        }

        private const int kMeshColliderNumSides = 8;

        private VolumetricLightBeam m_Beam = null;
        private DynamicOcclusionRaycasting m_DynamicOcclusionRaycasting = null;
        ...
        private void ComputeZone()
        {
            if (m_Beam)
            {
                ...
                var lerpedRadiusEnd =
                    Mathf.LerpUnclamped(m_Beam.coneRadiusStart, m_Beam.coneRadiusEnd, rangeMultiplier);
                ...
                if (m_Beam.dimensions == Dimensions.Dim3D)
                {
                    ...
                    var sides = Mathf.Min(m_Beam.geomSides, kMeshColliderNumSides);
                    var mesh = MeshGenerator.GenerateConeZ_Radius(rangeEnd, m_Beam.coneRadiusStart, lerpedRadiusEnd,
                        sides, 0, false, false);
