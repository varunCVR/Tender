...
namespace VLB
{
    ...
    {
        private SerializedProperty geometryOverrideLayer = null,
            geometryLayerID = null,
            geometryTag = null,
            geometryRenderQueue = null,
            renderPipeline = null,
            renderingMode = null;

        private SerializedProperty sharedMeshSides = null, sharedMeshSegments = null;
        private SerializedProperty globalNoiseScale = null, globalNoiseVelocity = null;
        private SerializedProperty fadeOutCameraTag = null;
        private SerializedProperty noiseTexture3D = null;
        private SerializedProperty dustParticlesPrefab = null;
        private SerializedProperty ditheringFactor = null, ditheringNoiseTexture = null;

        private SerializedProperty featureEnabledColorGradient = null,
            featureEnabledDepthBlend = null,
            featureEnabledNoise3D = null,
            featureEnabledDynamicOcclusion = null,
            featureEnabledMeshSkewing = null,
            featureEnabledShaderAccuracyHigh = null;

        private bool isRenderQueueCustom = false;
        private Config m_TargetConfig = null;
        ...
        protected override void OnEnable()
        {
            ...
            m_TargetConfig = target as Config;
        }

        private void RenderQueueGUIInit()
        {
        ...
        private void RenderQueueGUIDraw()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    var rq = isRenderQueueCustom ? RenderQueue.Custom : (RenderQueue) geometryRenderQueue.intValue;
                    ...
                    {
                        geometryRenderQueue.intValue =
                            EditorGUILayout.IntField(geometryRenderQueue.intValue, GUILayout.MaxWidth(65.0f));
                    }
                    EditorGUI.EndDisabledGroup();
                }
                if (EditorGUI.EndChangeCheck()) SetDirty(DirtyFlags.AllBeamGeom);
            }
        }
        ...
        protected override void OnHeaderGUI()
        {
            ...
            EditorGUILayout.LabelField(string.Format("Current Version: {0}", Version.Current),
                EditorStyles.miniBoldLabel);
            EditorGUILayout.Separator();
            GUILayout.EndVertical();
        }

        private void OnAddConfigPerPlatform(object platform)
        {
            var p = (RuntimePlatform) platform;
            var clone = Instantiate(m_TargetConfig);
            Debug.Assert(clone);
        ...
        private void SetDirty(DirtyFlags flags)
        {
            if (m_IsUsedInstance)
            {
        ...
        private bool m_NeedToReloadNoise = false;
        private bool m_NeedToRefreshShader = false;
        private bool m_IsUsedInstance = false;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var hasValidName = m_TargetConfig.HasValidAssetName();
                var isCurrentPlatformSuffix =
                    m_TargetConfig.GetAssetSuffix() == PlatformHelper.GetCurrentPlatformSuffix();

                var platformSuffix = m_TargetConfig.GetAssetSuffix();
                var platformStr = "Default Config asset";
...

            {
                EditorGUI.BeginChangeCheck();
                {
                    if (FoldableHeader.Begin(this, EditorStrings.Config.HeaderBeamGeometry))
                    {
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            geometryOverrideLayer.boolValue = EditorGUILayout.Toggle(
                                EditorStrings.Config.GeometryOverrideLayer, geometryOverrideLayer.boolValue);
                        ...
                        geometryTag.stringValue =
                            EditorGUILayout.TagField(EditorStrings.Config.GeometryTag, geometryTag.stringValue);
                    }
                    ...
                    if (FoldableHeader.Begin(this, EditorStrings.Config.HeaderRendering))
                    {
                        ...
                        if (BeamGeometry.isCustomRenderPipelineSupported)
                        {
                            EditorGUI.BeginChangeCheck();
                            {
                                renderPipeline.CustomEnum<RenderPipeline>(EditorStrings.Config.GeometryRenderPipeline,
                                    EditorStrings.Config.GeometryRenderPipelineEnumDescriptions);
                            }
                            if (EditorGUI.EndChangeCheck())
                                SetDirty(DirtyFlags.AllBeamGeom |
                                         DirtyFlags.Shader); // need to fully reset the BeamGeom to update the shader
                        }

                        if (m_TargetConfig.hasRenderPipelineMismatch)
                            EditorGUILayout.HelpBox(EditorStrings.Config.ErrorRenderPipelineMismatch,
                                MessageType.Error);
                        ...
                        EditorGUI.BeginChangeCheck();
                        {
                            ...
                            if (renderPipeline.enumValueIndex == (int) RenderPipeline.BuiltIn)
                            {
                                if (renderingMode.enumValueIndex == (int) RenderingMode.SRPBatcher)
                                    EditorGUILayout.HelpBox(EditorStrings.Config.ErrorSrpBatcherOnlyCompatibleWithSrp,
                                        MessageType.Error);
                            }
                            else
                            {
                                if (renderingMode.enumValueIndex == (int) RenderingMode.SRPBatcher &&
                                    SRPHelper.renderPipelineType == SRPHelper.RenderPipeline.LWRP)
                                    EditorGUILayout.HelpBox(EditorStrings.Config.ErrorSrpBatcherNotCompatibleWithLWRP,
                                        MessageType.Error);

                                if (renderingMode.enumValueIndex == (int) RenderingMode.MultiPass)
                                    EditorGUILayout.HelpBox(EditorStrings.Config.ErrorSrpAndMultiPassNotCompatible,
                                        MessageType.Error);
                            }

#pragma warning disable 0162 // warning CS0162: Unreachable code detected
                            if (renderingMode.enumValueIndex == (int) RenderingMode.GPUInstancing &&
                                !BatchingHelper.isGpuInstancingSupported)
                                EditorGUILayout.HelpBox(EditorStrings.Config.ErrorGeometryGpuInstancingNotSupported,
                                    MessageType.Error);
#pragma warning restore 0162
                        }
                        if (EditorGUI.EndChangeCheck())
                            SetDirty(DirtyFlags.AllBeamGeom | DirtyFlags.GlobalMesh |
                                     DirtyFlags.Shader); // need to fully reset the BeamGeom to update the shader
                ...
                if (FoldableHeader.Begin(this, EditorStrings.Config.HeaderSharedMesh))
                {
                    ...
                    if (EditorGUI.EndChangeCheck()) SetDirty(DirtyFlags.GlobalMesh | DirtyFlags.AllMeshes);

                    var meshInfo =
                        "These properties will change the mesh tessellation of each Volumetric Light Beam with 'Shared' MeshType.\nAdjust them carefully since they could impact performance.";
                    meshInfo += string.Format("\nShared Mesh stats: {0} vertices, {1} triangles",
                        MeshGenerator.GetSharedMeshVertexCount(), MeshGenerator.GetSharedMeshIndicesCount() / 3);
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    fadeOutCameraTag.stringValue = EditorGUILayout.TagField(EditorStrings.Config.FadeOutCameraTag,
                        fadeOutCameraTag.stringValue);
                ...
                if (FoldableHeader.Begin(this, EditorStrings.Config.HeaderFeaturesEnabled))
                {
                    EditorGUI.BeginChangeCheck();
                    {
                        EditorGUILayout.PropertyField(featureEnabledColorGradient,
                            EditorStrings.Config.FeatureEnabledColorGradient);
                        EditorGUILayout.PropertyField(featureEnabledDepthBlend,
                            EditorStrings.Config.FeatureEnabledDepthBlend);
                        EditorGUILayout.PropertyField(featureEnabledNoise3D,
                            EditorStrings.Config.FeatureEnabledNoise3D);
                        EditorGUILayout.PropertyField(featureEnabledDynamicOcclusion,
                            EditorStrings.Config.FeatureEnabledDynamicOcclusion);
                        EditorGUILayout.PropertyField(featureEnabledMeshSkewing,
                            EditorStrings.Config.FeatureEnabledMeshSkewing);
                        EditorGUILayout.PropertyField(featureEnabledShaderAccuracyHigh,
                            EditorStrings.Config.FeatureEnabledShaderAccuracyHigh);
                    }
                    if (EditorGUI.EndChangeCheck()) SetDirty(DirtyFlags.Shader | DirtyFlags.AllBeamGeom);
                }
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        Undo.RecordObject(target, "Reset Internal Data");
            ...
            if (m_NeedToRefreshShader)
                m_TargetConfig.RefreshShader(Config.RefreshShaderFlags
                    .All); // need to be done AFTER ApplyModifiedProperties
