...
namespace VLB
{
    ...
    {
        private SerializedProperty updateRate = null;
        private SerializedProperty waitXFrames = null;
        ...
        protected void DisplayCommonInspector()
        {
            ...
            {
                updateRate.CustomEnum<DynamicOcclusionUpdateRate>(EditorStrings.DynOcclusion.UpdateRate,
                    EditorStrings.DynOcclusion.UpdateRateDescriptions);

                if (m_Targets.HasAtLeastOneTargetWith((T comp) =>
                    {
                        return comp.updateRate.HasFlag(DynamicOcclusionUpdateRate.EveryXFrames);
                    })) EditorGUILayout.PropertyField(waitXFrames, EditorStrings.DynOcclusion.WaitXFrames);

                EditorGUILayout.HelpBox(
                    string.Format(EditorStrings.DynOcclusion.GetUpdateRateAdvice<T>(m_Targets[0].updateRate),
                        m_Targets[0].waitXFrames),
                    MessageType.Info);
            }
