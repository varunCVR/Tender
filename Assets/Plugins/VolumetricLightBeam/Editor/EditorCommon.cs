...
namespace VLB
{
    public class EditorCommon : Editor
    {
        ...
        private static void DrawLineSeparator(Color color, int thickness = 2, int padding = 10)
        {
        ...
        protected void ButtonOpenConfig(bool miniButton = true)
        {
            ...
            if (miniButton)
                buttonClicked = GUILayout.Button(EditorStrings.Common.ButtonOpenGlobalConfig, EditorStyles.miniButton);
        ...
        protected bool GlobalToggleButton(ref bool boolean, GUIContent content, string saveString,
            float maxWidth = 999.0f)
        {
        ...
        private string GetThisNamespaceAsString()
        {
            return GetType().Namespace;
        }

        private SerializedProperty FindProperty(string prefix, string name)
        {
            Debug.Assert(serializedObject != null);
            var prop = serializedObject.FindProperty(name);
            if (prop == null)
            {
                name = string.Format("{0}{1}{2}", prefix, char.ToUpperInvariant(name[0]),
                    name.Substring(1)); // get '_Name' from 'name'
        ...
        private void RetrieveSerializedProperties(string prefix, Type t)
        {
            if (t == null) return;
            ...
            foreach (var field in allEditorFields)
                if (field.FieldType == typeof(SerializedProperty))
                {
                    ...
                    Debug.AssertFormat(serializedProp != null, "Fail to find serialized field '{0}' in object {1}",
                        runtimeFieldName, serializedObject.targetObject);
        ...
        public abstract class EditorGUIWidth : IDisposable
        {
            protected abstract void ApplyWidth(float width);

            public EditorGUIWidth(float width)
            {
                ApplyWidth(width);
            }
        ...
        public class LabelWidth : EditorGUIWidth
        {
            ...
            protected override void ApplyWidth(float width)
            {
