...
namespace VLB
{
    ...
    {
        private SerializedProperty trackChangesDuringPlaytime = null;
        private SerializedProperty colorFromLight = null, colorMode = null, color = null, colorGradient = null;

        private SerializedProperty intensityFromLight = null,
            intensityModeAdvanced = null,
            intensityInside = null,
            intensityOutside = null;

        private SerializedProperty blendingMode = null, shaderAccuracy = null;
        private SerializedProperty fresnelPow = null, glareFrontal = null, glareBehind = null;
        private SerializedProperty spotAngleFromLight = null, spotAngle = null;

        private SerializedProperty coneRadiusStart = null,
            geomMeshType = null,
            geomCustomSides = null,
            geomCustomSegments = null,
            geomCap = null;

        private SerializedProperty fallOffEndFromLight = null, fallOffStart = null, fallOffEnd = null;
        private SerializedProperty attenuationEquation = null, attenuationCustomBlending = null;
        private SerializedProperty depthBlendDistance = null, cameraClippingDistance = null;

        private SerializedProperty noiseMode = null,
            noiseIntensity = null,
            noiseScaleUseGlobal = null,
            noiseScaleLocal = null,
            noiseVelocityUseGlobal = null,
            noiseVelocityLocal = null;

        private SerializedProperty fadeOutBegin = null, fadeOutEnd = null;
        private SerializedProperty dimensions = null, sortingLayerID = null, sortingOrder = null;

        private SerializedProperty skewingLocalForwardDirection = null,
            clippingPlaneTransform = null,
            tiltFactor = null;

        private TargetList<VolumetricLightBeam> m_Targets;
        ...
        private static void PropertyThickness(SerializedProperty sp)
        {
            sp.FloatSlider(
        ...
        private class ButtonToggleScope : System.IDisposable
        {
            private SerializedProperty m_Property;
            private bool m_DisableGroup = false;
            ...
            private static GUIStyle ms_ToggleButtonStyleNormal = null;
            private static GUIStyle ms_ToggleButtonStyleToggled = null;
            private static GUIStyle ms_ToggleButtonStyleMixedValue = null;
            ...
            private void DrawToggleButton()
            {
                ...
                var style = EditorGUI.showMixedValue ? ms_ToggleButtonStyleMixedValue :
                    m_Property.boolValue ? ms_ToggleButtonStyleToggled : ms_ToggleButtonStyleNormal;
        ...
        private static ButtonToggleScope ButtonToggleScopeFromLight(SerializedProperty prop, bool visible)
        {
        ...
        private static ButtonToggleScope ButtonToggleScopeAdvanced(SerializedProperty prop, bool visible)
        {
            if (!visible) return null;
        ...
        public override void OnInspectorGUI()
        {
            ...
            var hasLightSpot = m_Targets[0].GetLightSpotAttachedSlow(out lightType) != null;
            ...
            if (FoldableHeader.Begin(this, EditorStrings.Beam.HeaderBasic))
            {
                ...
                using (ButtonToggleScopeFromLight(colorFromLight, hasLightSpot))
                {
                    if (!hasLightSpot)
                        EditorGUILayout
                            .BeginHorizontal(); // mandatory to have the color picker on the same line (when the button "from light" is not here)
                    {
                ...
                // Intensity
                var advancedModeEnabled = false;
                using (ButtonToggleScopeFromLight(intensityFromLight, hasLightSpot))
                {
                    var advancedModeButton = !hasLightSpot || intensityFromLight.HasAtLeastOneValue(false);
                    using (ButtonToggleScopeAdvanced(intensityModeAdvanced, advancedModeButton))
                    {
                        ...
                        EditorGUILayout.PropertyField(intensityOutside,
                            advancedModeEnabled
                                ? EditorStrings.Beam.IntensityOutside
                                : EditorStrings.Beam.IntensityGlobal);
                    }
                }
            ...
            if (FoldableHeader.Begin(this, EditorStrings.Beam.HeaderAttenuation))
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (attenuationEquation.enumValueIndex == (int) AttenuationEquation.Blend)
                        EditorGUILayout.PropertyField(attenuationCustomBlending,
                            EditorStrings.Beam.AttenuationCustomBlending);
                }
                EditorGUILayout.EndHorizontal();
                ...
                if (fallOffEnd.hasMultipleDifferentValues)
                    ...
                    fallOffStart.FloatSlider(EditorStrings.Beam.FallOffStart, 0f,
                        fallOffEnd.floatValue - Consts.Beam.FallOffDistancesMinThreshold);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    GlobalToggleButton(ref VolumetricLightBeam.editorShowTiltFactor,
                        EditorStrings.Beam.EditorShowTiltDirection, EditorPrefsStrings.Beam.PrefShowTiltDir, 50f);
                }

                if (m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                    {
                        return beam.isTilted && beam.shaderAccuracy != ShaderAccuracy.High;
                    }))
            ...
            if (Config.Instance.featureEnabledNoise3D)
            {
                ...
                {
                    noiseMode.CustomEnum<NoiseMode>(EditorStrings.Beam.NoiseMode,
                        EditorStrings.Beam.NoiseModeEnumDescriptions);

                    var showNoiseProps = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                    {
                        return beam.isNoiseEnabled;
                    });
                    if (showNoiseProps)
                    {
            ...
            {
                {
                    var content = AddEnabledStatusToContentText(EditorStrings.Beam.CameraClippingDistance,
                ...
                {
                    var content =
                        AddEnabledStatusToContentText(EditorStrings.Beam.DepthBlendDistance, depthBlendDistance);
            ...
            if (FoldableHeader.Begin(this, EditorStrings.Beam.HeaderGeometry))
            {
                ...
                if (geomMeshType.intValue == (int) MeshType.Custom)
                {
                    ...
                    if (Config.Instance.featureEnabledMeshSkewing)
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        {
                            vec2 = EditorGUILayout.Vector2Field(EditorStrings.Beam.SkewingLocalForwardDirection,
                                vec3.xy());
                        }
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    if (m_Targets.Count == 1) EditorGUILayout.HelpBox(m_Targets[0].meshStats, MessageType.Info);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                        {
                            return beam.clippingPlaneTransform != null;
                        }))
                        GlobalToggleButton(ref VolumetricLightBeam.editorShowClippingPlane,
                            EditorStrings.Beam.EditorShowClippingPlane, EditorStrings.Beam.PrefShowAddClippingPlane,
                            50f);
                }
            }
            ...
            {
                var wasEnabled = fadeOutBegin.floatValue <= fadeOutEnd.floatValue;

                if (m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                    {
                        return beam.fadeOutBegin <= beam.fadeOutEnd != wasEnabled;
                    }))
                {
                    wasEnabled = true;
                ...
                EditorGUI.BeginChangeCheck();
                var isEnabled = EditorGUILayout.Toggle(EditorStrings.Beam.FadeOutEnabled, wasEnabled);
                EditorGUI.showMixedValue = false;
                if (EditorGUI.EndChangeCheck())
                {
                    float invValue = isEnabled ? 1 : -1;
                    var valueA = Mathf.Abs(fadeOutBegin.floatValue);
                    var valueB = Mathf.Abs(fadeOutEnd.floatValue);
            ...
            {
                dimensions.CustomEnum<Dimensions>(EditorStrings.Beam.Dimensions,
                    EditorStrings.Common.DimensionsEnumDescriptions);
        ...
        private GUIContent AddEnabledStatusToContentText(GUIContent inContent, SerializedProperty prop)
        {
        ...
        private void SetMeshesDirty()
        {
            foreach (var entity in m_Targets) entity._EditorSetMeshDirty();
        }

        private void DrawSortingLayerAndOrder()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUI.BeginChangeCheck();
                ...
                var layerIndex =
                    System.Array.IndexOf(m_SortingLayerNames, SortingLayer.IDToName(sortingLayerID.intValue));
        ...
        private void DrawAnimatorWarning()
        {
            var showAnimatorWarning = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
            {
                return beam.GetComponent<Animator>() != null && beam.trackChangesDuringPlaytime == false;
            });
        ...
        private void DrawEditInSceneButton()
        {
            EditorGUI.BeginChangeCheck();
            var editInScene = EditorPrefs.GetBool(EditorStrings.Beam.PrefEditInScene, false);
        ...
        private void DrawCustomActionButtons()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button(EditorStrings.Beam.ButtonResetProperties, EditorStyles.miniButton))
                    m_Targets.RecordUndoAction("Reset Light Beam Properties",
                        ...
                        {
                            beam.Reset();
                            beam.GenerateGeometry();
                        });

                if (m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                    {
                        return beam.geomMeshType == MeshType.Custom;
                    }))
                    if (GUILayout.Button(EditorStrings.Beam.ButtonGenerateGeometry, EditorStyles.miniButton))
                        foreach (var entity in m_Targets)
                            entity.GenerateGeometry();
            }
        }

        private void AddComponentToTargets<T>() where T : MonoBehaviour
        {
            foreach (var target in m_Targets) EditorExtensions.AddComponentFromEditor<T>(target);
        }

        private void DrawAdditionalFeatures()
        {
            if (Application.isPlaying) return; // do not support adding additional components at runtime

            var showButtonDust = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
            {
                return beam.GetComponent<VolumetricDustParticles>() == null;
            });
            var showButtonOcclusion = Config.Instance.featureEnabledDynamicOcclusion &&
                                      m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
                                      {
                                          return beam.GetComponent<DynamicOcclusionAbstractBase>() == null;
                                      });
            var showButtonTriggerZone = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
            {
                return beam.GetComponent<TriggerZone>() == null;
            });
            var showButtonEffect = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) =>
            {
                return beam.GetComponent<EffectAbstractBase>() == null;
            });

            using (new EditorGUILayout.HorizontalScope())
            {
                var kStyle = GUI.skin.button;
                ...
                if (showButtonOcclusion && GUILayout.Button(EditorData.Instance.contentAddDynamicOcclusion, kStyle))
                {
                    ...
                    menu.AddItem(new GUIContent(EditorStrings.Beam.ButtonAddDynamicOcclusionRaycasting), false,
                        AddComponentToTargets<DynamicOcclusionRaycasting>);
                    menu.AddItem(new GUIContent(EditorStrings.Beam.ButtonAddDynamicOcclusionDepthBuffer), false,
                        AddComponentToTargets<DynamicOcclusionDepthBuffer>);
                    menu.ShowAsContext();
                }
                ...
                {
                    var menu = new GenericMenu();
                    menu.AddItem(new GUIContent(EditorStrings.Beam.ButtonAddEffectFlicker), false,
                        AddComponentToTargets<EffectFlicker>);
                    menu.AddItem(new GUIContent(EditorStrings.Beam.ButtonAddEffectPulse), false,
                        AddComponentToTargets<EffectPulse>);
                    menu.ShowAsContext();
                }
        ...
        private struct InfoTip
        {
            public MessageType type;
            public string message;
        }

        private bool DrawInfos()
        {
        ...
        private List<InfoTip> GetInfoTips()
        {
            ...
            if (m_Targets.Count == 1)
                if (depthBlendDistance.floatValue > 0f || !Noise3D.isSupported || trackChangesDuringPlaytime.boolValue)
                {
                    ...
                    {
                        tips.Add(new InfoTip
                            {type = MessageType.Info, message = EditorStrings.Beam.HelpDepthTextureMode});
                    ...
                    if (trackChangesDuringPlaytime.boolValue)
                        tips.Add(new InfoTip
                            {type = MessageType.Info, message = EditorStrings.Beam.HelpTrackChangesEnabled});
                }

            return tips;
        }

        private string GetBatchingReport()
        {
            if (m_Targets.Count > 1)
            {
                var reasons = "";
        ...
        private void DrawFallOffSliderHandle(VolumetricLightBeam beam, float lineStartOffset, float lineAlpha,
            Handles.CapFunction capFunction, string recordName, ref float value)
        {
            ...
            sliderPos = Handles.Slider(sliderPos, fwd, HandleUtility.GetHandleSize(sliderPos) * 0.1f, capFunction,
                0.5f);
        ...
        private void DrawRadiusHandle(VolumetricLightBeam beam, Vector3 pos, string recordName, float value,
            System.Action<float> setValue)
        {
            ...
            var handleMatrix = Matrix4x4.TRS(
                pos,
                Quaternion.LookRotation(beam.transform.up, beam.beamGlobalForward) *
                Quaternion.Euler(0.0f, 45.0f,
                    0.0f), // rotate the circle handle to have both this and the Unity's light handle
                Vector3.one);
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            {
                DrawRadiusHandle(beam, beam.transform.position, "Truncated Radius", beam.coneRadiusStart,
                    (float v) => beam.coneRadiusStart = v);
            }

            if (!beam.hasMeshSkewing)
            {
                Handles.color = beam.ComputeColorAtDepth(beam.fallOffStart / beam.fallOffEnd)
                    .ComputeComplementaryColor(true);
                {
                    DrawFallOffSliderHandle(beam, 0.0f, 1.0f, Handles.CylinderHandleCap, "Start Distance",
                        ref beam.fallOffStart);
                }
                ...
                Handles.color = beam.ComputeColorAtDepth(1.0f).ComputeComplementaryColor(true);
                {
                    ...
                    if (!hasSpotLight || !beam.fallOffEndFromLight)
                        DrawFallOffSliderHandle(beam, beam.fallOffStart, 0.5f, Handles.CubeHandleCap, "Range Limit",
                            ref beam.fallOffEnd);

                    if (!hasSpotLight || !beam.spotAngleFromLight)
                    {
                        var fallOffEndPos =
                            beam.transform.localToWorldMatrix.MultiplyPoint(beam.beamLocalForward * beam.fallOffEnd);
                        DrawRadiusHandle(beam, fallOffEndPos, "Spot Angle", beam.coneRadiusEnd,
                            (float v) => beam.coneRadiusEnd = v);
                    }
                }
            }
        }
