...
namespace VLB
{
    ...
    {
        private SerializedProperty dimensions = null;
        private SerializedProperty layerMask = null;
        private SerializedProperty considerTriggers = null;
        private SerializedProperty minOccluderArea = null;
        private SerializedProperty planeAlignment = null;
        private SerializedProperty maxSurfaceDot = null;
        private SerializedProperty planeOffset = null;
        private SerializedProperty fadeDistanceToSurface = null;
        private SerializedProperty minSurfaceRatio = null;

        public override bool RequiresConstantRepaint()
        {
            return Application.isPlaying || DynamicOcclusionRaycasting.editorRaycastAtEachFrame;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                dimensions.CustomEnum<Dimensions>(EditorStrings.DynOcclusion.Dimensions,
                ...
                if (Physics2D.queriesHitTriggers == false)
                    if (m_Targets.HasAtLeastOneTargetWith((DynamicOcclusionRaycasting instance) =>
                        {
                            return instance.dimensions == Dimensions.Dim2D && instance.considerTriggers;
                        }))
                        EditorGUILayout.HelpBox(EditorStrings.DynOcclusion.ConsiderTriggersNoPossible,
                            MessageType.Error);
            ...
            if (FoldableHeader.Begin(this, EditorStrings.DynOcclusion.HeaderEditorDebug))
            {
                ...
                {
                    GlobalToggleButton(ref DynamicOcclusionRaycasting.editorShowDebugPlane,
                        EditorStrings.DynOcclusion.EditorShowDebugPlane,
                        EditorPrefsStrings.DynOcclusion.PrefShowDebugPlane);
                    GlobalToggleButton(ref DynamicOcclusionRaycasting.editorRaycastAtEachFrame,
                        EditorStrings.DynOcclusion.EditorRaycastAtEachFrame,
                        EditorPrefsStrings.DynOcclusion.PrefRaycastingEditor);
                }
                ...
                if (Application.isPlaying || DynamicOcclusionRaycasting.editorRaycastAtEachFrame)
                    if (!serializedObject.isEditingMultipleObjects)
                    {
                        ...
                        occluderInfo += hit.hasCollider
                            ? string.Format("Current occluder: '{0}'\nEstimated occluder area: {1} units²", hit.name,
                                hit.bounds.GetMaxArea2D())
                            : "No occluder found";
