		...
		if (this.IsBound())
		{
			...
			else
			{
				...
				case RigidbodyInterpolation.Interpolate:
					this.UpdatePosition(_boundRigid.position + (_boundRigid.linearVelocity * Time.fixedDeltaTime) / 2);
					break;
				case RigidbodyInterpolation.None:
				default:
					this.UpdatePosition(_boundRigid.position + _boundRigid.linearVelocity * Time.fixedDeltaTime);
					break;
				}
			}
		}
